...
public class WebParser {
	...
	public List<String> getSearchUrl() {
		...
		for (String type : new String[]{"sii", "otc"}) {
			...
			paramMap.put("season", "0" + this.season);
	...
	public Document search(String url, Integer maxTryTimes) throws InterruptedException {
			...
			for (Element tr : trs) {
				if (tr.hasClass("tblHead")) {
					...
					for (int idx = 0; idx < ths.size(); idx++) {
						...
						if (th.text().contains("每股稅後盈餘") || th.text().contains("每股盈餘")) {
							epsIdx = idx;
							break;
						}
					}
					if (epsIdx == null) {
						log.warn("epsIdx is null, tr = {}", tr);
					}
				} else {
		...
		for (Element table : tables) {
			...
			for (Element tr : trs) {
				if (tr.hasClass("tblHead")) {
					...
					for (int idx = 0; idx < ths.size(); idx++) {
						Element th = ths.get(idx);
						if (th.text().contains("基本每股盈餘") || th.text().contains("每股盈餘")) {
							epsIdx = idx;
							break;
						}
					}
